classdef SO3VariationMPC < Controller
properties
    kp_att = [8; 8; 3];
    kp_Om = [0.3; 0.3; 0.225];
    kd_Om = zeros(3, 1);
...
%%
methods
    function obj = SO3VariationMPC(varargin)
        ...
        % input constraints
        obj.Au = [-eye(3); eye(3)];
        obj.bu = [obj.Mmin; obj.Mmax];
        
        % state constraints
        obj.Ax = [-eye(3); eye(3)];
        obj.bx = [inf*ones(3,1); inf*ones(3,1)];
    ...
    function [u, moment] = apply(obj,t, F, R, Omega)
        ...
        [H, f, ~] = generate_mpc_qp(N, eta0, Ak, Bk, Q1, S, Q2);
        LB = kron(ones(N,1), obj.Mmin);
        UB = kron(ones(N,1), obj.Mmax);
        
        [xOpt, ~, ~] = qp(H, f, [], [], LB, UB, obj.x0);
        obj.x0 = xOpt;
        M = xOpt(1:3);
        moment = M + cmd.M;
