    ...
    if (qrmanager->ncols <= 0) {
      ...
      case 3: {
        ...
        if (alwaysPositiveDef) {
          ...
          if (qrmanager->mrows < 1) {
            ...
            if (qrmanager->mrows > 1) {
              ...
              for (k = 2; k <= nVar_tmp + 1; k++) {
                double s;
                s = std::abs(cholmanager->FMat[(k - 1) * 12]);
                if (s > smax) {
                  nVars = k - 1;
                  smax = s;
                }
          ...
          cholmanager->regTol_ =
              std::fmax(std::abs(cholmanager->FMat[nVars + 11 * nVars]) *
          ...
          if (cholmanager->ConvexCheck) {
            ...
            do {
              exitg1 = 0;
              if (idx <= nVar_tmp) {
                if (cholmanager->FMat[idx + 11 * idx] <= 0.0) {
        ...
        if (cholmanager->info != 0) {
          ...
          i = cholmanager->ndims;
          for (idx = 0; idx < i; idx++) {
            solution->searchDir[idx] /= cholmanager->FMat[idx + 11 * idx];
          }
      ...
      nullStartIdx_tmp = 11 * qrmanager->ncols + 1;
      switch (objective->objtype) {
      case 5: {
        for (idx = 0; idx < mNull_tmp; idx++) {
          memspace->workspace_double[idx] =
              -qrmanager->Q[nVar_tmp + 11 * (qrmanager->ncols + idx)];
        }
        if (qrmanager->mrows != 0) {
          ...
          ix = 0;
          i = nullStartIdx_tmp + 11 * (mNull_tmp - 1);
          for (idx = nullStartIdx_tmp; idx <= i; idx += 11) {
            int idx_row;
          ...
          if (alwaysPositiveDef) {
            ...
            for (ix = 0; ix < mNull_tmp; ix++) {
              ...
              for (idx_row = i; idx_row <= nVars; idx_row++) {
                memspace->workspace_double[(idx_row + 11 * ix) - 1] =
                    objective->beta *
                    qrmanager->Q[(idx_row + 11 * (ix + qrmanager->ncols)) - 1];
              }
            }
            internal::blas::xgemm(mNull_tmp, mNull_tmp, qrmanager->mrows,
                                  qrmanager->Q, nullStartIdx_tmp,
        ...
        if (cholmanager->info != 0) {
          ...
          if (qrmanager->mrows != 0) {
            ...
            for (idx = nullStartIdx_tmp; idx <= i; idx += 11) {
              ...
              idx_row = div_nde_s32_floor(idx - nullStartIdx_tmp, 11);
