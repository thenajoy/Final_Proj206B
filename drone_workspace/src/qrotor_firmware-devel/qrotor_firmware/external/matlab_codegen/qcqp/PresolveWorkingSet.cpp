  ...
  if (mTotalWorkingEq_tmp_tmp > 0) {
    ...
    for (ix = 0; ix < mTotalWorkingEq_tmp_tmp; ix++) {
      for (idx_col = 0; idx_col <= nVar; idx_col++) {
        qrmanager->QR[ix + 11 * idx_col] = workingset->ATwset[idx_col + 6 * ix];
      }
    }
    ...
    if (nDepInd > 0) {
      ...
      while ((!exitg1) && (idx_col <= nDepInd - 1)) {
        double qtb;
        ix = 11 * ((mTotalWorkingEq_tmp_tmp - idx_col) - 1);
        qtb = 0.0;
          ...
          if (ix0 <= i) {
            if ((workingset->nActiveConstr == i) || (ix0 == i)) {
              ...
              for (u0 = 0; u0 < ix0; u0++) {
                workingset
                    ->ATwset[u0 +
                             6 * (memspace->workspace_int[idx_col - 1] - 1)] =
                    workingset
                        ->ATwset[u0 + 6 * (workingset->nActiveConstr - 1)];
              }
              workingset->bwset[i] =
              ...
              for (u0 = 0; u0 < idxDiag; u0++) {
                workingset->ATwset[u0 + 6 * (ix0 - 1)] =
                    workingset->ATwset[u0 + 6 * (i - 1)];
              }
