...
// Source: https://github.com/PX4/PX4-Autopilot/blob/f9d8c613b048f58eb3110e9af13cb3a89c4c866f/src/modules/mc_pos_control/PositionControl/ControlMath.cpp
//
...
void limitTilt(Vector3f &body_unit, const Vector3f &world_unit, const float max_angle) {
  ...
  float angle = std::acos(dot_product_unit);
  // limit tilt
...
void addIfNotNan(float &setpoint, const float addition) {
  if (std::isfinite(setpoint) && std::isfinite(addition)) {
    // No NAN, add to the setpoint
    setpoint += addition;

  } else if (!std::isfinite(setpoint)) {
